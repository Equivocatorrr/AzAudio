...
void azaBufferDeinterlace2Ch_scalar(azaBuffer dst, azaBuffer src) {
	for (uint32_t i = 0; i < dst.frames*2; i+=2) {
		dst.samples[(i>>1)] = src.samples[i];
		dst.samples[dst.frames + (i>>1)] = src.samples[i + 1];
	}
}
...
void azaBufferDeinterlace2Ch_sse(azaBuffer dst, azaBuffer src) {
	int32_t i = 0;
#if 0
	for (; i <= (int32_t)src.frames*2-4; i += 4) {
		__m128 AB = _mm_loadu_ps(src.samples+i);
		AB = _mm_shuffle_ps(AB, AB, _MM_SHUFFLER(0, 2, 1, 3));
		_mm_storel_pi((__m64*)(dst.samples+(i>>1)), AB);
		_mm_storeh_pi((__m64*)(dst.samples+dst.frames+(i>>1)), AB);
	}
	for (; i < (int32_t)src.frames*2; i+=2) {
		dst.samples[(i>>1)] = src.samples[i];
		dst.samples[dst.frames + (i>>1)] = src.samples[i + 1];
	}
#else
...
void azaBufferDeinterlace2Ch_avx2(azaBuffer dst, azaBuffer src) {
	...
	for (; i <= (int32_t)src.frames-4; i += 4) {
		int32_t f = i << 1;
		__m256 AB = _mm256_loadu_ps(src.samples + f);
		AB = _mm256_permutevar8x32_ps(AB, permute);
		__m128 A = _mm256_extractf128_ps(AB, 0);
		_mm_storeu_ps(dst.samples + i, A);
		__m128 B = _mm256_extractf128_ps(AB, 1);
		_mm_storeu_ps(dst.samples + dst.frames + i, B);
	}
#else
	float *const dst_samples_a = dst.samples;
	float *const dst_samples_b = dst.samples + dst.frames;
...
void azaBufferDeinterlace3Ch_scalar(azaBuffer dst, azaBuffer src) {
	for (uint32_t i = 0; i < dst.frames; i++) {
		uint32_t f = i * 3;
		dst.samples[i] = src.samples[f++];
		dst.samples[dst.frames + i] = src.samples[f++];
		dst.samples[(dst.frames<<1) + i] = src.samples[f];
	}
}
...
#if 0
		// This is better than scalar, but not by much.
		__m128 B0_C0_A0_A1 = _mm_shuffle_ps(A0_B0_C0_A1, A0_B0_C0_A1, _MM_SHUFFLER(1, 2, 0, 3));
		__m128 C0_B0_A0_A1 = _mm_shuffle_ps(A0_B0_C0_A1, A0_B0_C0_A1, _MM_SHUFFLER(2, 1, 0, 3));

		__m128 C1_A2_B1_B2 = _mm_shuffle_ps(B1_C1_A2_B2, B1_C1_A2_B2, _MM_SHUFFLER(1, 2, 0, 3));
		__m128 A2_C1_B1_B2 = _mm_shuffle_ps(B1_C1_A2_B2, B1_C1_A2_B2, _MM_SHUFFLER(2, 1, 0, 3));

		__m128 A3_B3_C2_C3 = _mm_shuffle_ps(C2_A3_B3_C3, C2_A3_B3_C3, _MM_SHUFFLER(1, 2, 0, 3));
		__m128 B3_A3_C2_C3 = _mm_shuffle_ps(C2_A3_B3_C3, C2_A3_B3_C3, _MM_SHUFFLER(2, 1, 0, 3));

		_mm_storeh_pi((__m64*)(dst_samples_a + i), B0_C0_A0_A1); // store A0 and A1
		_mm_store_ss(dst_samples_a + i + 2, A2_C1_B1_B2); // store A2
		_mm_store_ss(dst_samples_a + i + 3, A3_B3_C2_C3); // store A3

		_mm_store_ss(dst_samples_b + i, B0_C0_A0_A1); // store B0
		_mm_storeh_pi((__m64*)(dst_samples_b + i + 1), C1_A2_B1_B2); // store B1 and B2
		_mm_store_ss(dst_samples_b + i + 3, B3_A3_C2_C3); // store B3

		_mm_store_ss(dst_samples_c + i, C0_B0_A0_A1); // store C0
		_mm_store_ss(dst_samples_c + i + 1, C1_A2_B1_B2); // store C1
		_mm_storeh_pi((__m64*)(dst_samples_c + i + 2), A3_B3_C2_C3); // store C2 and C3
#else
...
#if 0
		// This is actually slightly slower than sse4.1, probably because there's 6 permutes.
		__m256 A0_A1_A2____B0_B1_B2___ = _mm256_permutevar8x32_ps(
			A0_B0_C0_A1_B1_C1_A2_B2,
			_mm256_setr_epi32(0, 3, 6, 0, 1, 4, 7, 0)
		);
		__m256 _________A3_A4_A5______ = _mm256_permutevar8x32_ps(
			C2_A3_B3_C3_A4_B4_C4_A5,
			_mm256_setr_epi32(0, 0, 0, 1, 4, 7, 0, 0)
		);
		__m256 __________________A6_A7 = _mm256_permutevar8x32_ps(
			B5_C5_A6_B6_C6_A7_B7_C7,
			_mm256_setr_epi32(0, 0, 0, 0, 0, 0, 2, 5)
		);
		__m256 A0_A1_A2_A3_A4_A5______ = _mm256_blend_ps(
			A0_A1_A2____B0_B1_B2___, _________A3_A4_A5______, 0b00111000
		);
		__m256 A0_A1_A2_A3_A4_A5_A6_A7 = _mm256_blend_ps(
			A0_A1_A2_A3_A4_A5______, __________________A6_A7, 0b11000000
		);
		__m256 _________B3_B4_________ = _mm256_permutevar8x32_ps(
			C2_A3_B3_C3_A4_B4_C4_A5,
			_mm256_setr_epi32(0, 0, 0, 2, 5, 0, 0, 0)
		);
		__m256 ___B5_B6_B7____C5_C6_C7 = _mm256_permutevar8x32_ps(
			B5_C5_A6_B6_C6_A7_B7_C7,
			_mm256_setr_epi32(0, 0, 3, 6, 0, 1, 4, 7)
		);
		__m256 B0_B1_B2_______B5_B6_B7 = _mm256_permute2f128_ps(
			A0_A1_A2____B0_B1_B2___, ___B5_B6_B7____C5_C6_C7, _MM256_PERMUTE2F128(1, 2)
		);
		__m256 B0_B1_B2_B3_B4_B5_B6_B7 = _mm256_blend_ps(
			B0_B1_B2_______B5_B6_B7, _________B3_B4_________, 0b00011000
		);
		__m256 C0_C1__________________ = _mm256_permutevar8x32_ps(
			A0_B0_C0_A1_B1_C1_A2_B2,
			_mm256_setr_epi32(2, 5, 0, 0, 0, 0, 0, 0)
		);
		__m256 ______C2_C3_C4_________ = _mm256_permutevar8x32_ps(
			C2_A3_B3_C3_A4_B4_C4_A5,
			_mm256_setr_epi32(0, 0, 0, 3, 6, 0, 0, 0)
		);
		__m256 C0_C1_C2_C3_C4_________ = _mm256_blend_ps(
			C0_C1__________________, ______C2_C3_C4_________, 0b00011100
		);
		__m256 C0_C1_C2_C3_C4_C5_C6_C7 = _mm256_blend_ps(
			C0_C1_C2_C3_C4_________, ___B5_B6_B7____C5_C6_C7, 0b11100000
		);
#else
...
void azaBufferDeinterlace4Ch_scalar(azaBuffer dst, azaBuffer src) {
	for (uint32_t i = 0; i < dst.frames*4; i += 4) {
		dst.samples[(i>>2)] = src.samples[i];
		dst.samples[dst.frames + (i>>2)] = src.samples[i + 1];
		dst.samples[(dst.frames<<1) + (i>>2)] = src.samples[i + 2];
		dst.samples[(dst.frames*3) + (i>>2)] = src.samples[i + 3];
	}
}
...
void azaBufferDeinterlace4Ch_sse(azaBuffer dst, azaBuffer src) {
	...
	float *const dst_samples_b = dst.samples + dst.frames;
	float *const dst_samples_c = dst.samples + (dst.frames<<1);
	float *const dst_samples_d = dst.samples + dst.frames*3;
	for (; i <= (int32_t)src.frames-4; i += 4) {
		int32_t f = i << 2;
		float *src_samples = src.samples + f;
...
// This doesn't gain much performance over sse, I guess because we can't really avoid crossing 128-bit lanes.
// Also, 4 channels is something of a best-case scenario for sse I think.
AZA_SIMD_FEATURES("avx2")
...
#if 0
		// This is just BARELY faster than sse
		__m256 A0_B0_C0_D0_A2_B2_C2_D2 = _mm256_permute2f128_ps(
			A0_B0_C0_D0_A1_B1_C1_D1, A2_B2_C2_D2_A3_B3_C3_D3, _MM256_PERMUTE2F128(0, 2)
		);
		__m256 A1_B1_C1_D1_A3_B3_C3_D3 = _mm256_permute2f128_ps(
			A0_B0_C0_D0_A1_B1_C1_D1, A2_B2_C2_D2_A3_B3_C3_D3, _MM256_PERMUTE2F128(1, 3)
		);
		__m256 A0_A1_B0_B1_A2_A3_B2_B3 = _mm256_unpacklo_ps(A0_B0_C0_D0_A2_B2_C2_D2, A1_B1_C1_D1_A3_B3_C3_D3);
		__m256 C0_C1_D0_D1_C2_C3_D2_D3 = _mm256_unpackhi_ps(A0_B0_C0_D0_A2_B2_C2_D2, A1_B1_C1_D1_A3_B3_C3_D3);

		__m256 A4_B4_C4_D4_A6_B6_C6_D6 = _mm256_permute2f128_ps(
			A4_B4_C4_D4_A5_B5_C5_D5, A6_B6_C6_D6_A7_B7_C7_D7, _MM256_PERMUTE2F128(0, 2)
		);
		__m256 A5_B5_C5_D5_A7_B7_C7_D7 = _mm256_permute2f128_ps(
			A4_B4_C4_D4_A5_B5_C5_D5, A6_B6_C6_D6_A7_B7_C7_D7, _MM256_PERMUTE2F128(1, 3)
		);
		__m256 A4_A5_B4_B5_A6_A7_B6_B7 = _mm256_unpacklo_ps(A4_B4_C4_D4_A6_B6_C6_D6, A5_B5_C5_D5_A7_B7_C7_D7);
		__m256 C4_C5_D4_D5_C6_C7_D6_D7 = _mm256_unpackhi_ps(A4_B4_C4_D4_A6_B6_C6_D6, A5_B5_C5_D5_A7_B7_C7_D7);

		__m256 B4_B5_A4_A5_B6_B7_A6_A7 = _mm256_shuffle_ps(
			A4_A5_B4_B5_A6_A7_B6_B7, A4_A5_B4_B5_A6_A7_B6_B7, _MM_SHUFFLER(2, 3, 0, 1)
		);
		__m256 A0_A1_A4_A5_A2_A3_A6_A7 = _mm256_blend_ps(
			A0_A1_B0_B1_A2_A3_B2_B3, B4_B5_A4_A5_B6_B7_A6_A7, 0b11001100
		);
		__m256 B4_B5_B0_B1_B6_B7_B2_B3 = _mm256_blend_ps(
			A0_A1_B0_B1_A2_A3_B2_B3, B4_B5_A4_A5_B6_B7_A6_A7, 0b00110011
		);
		__m256 A0_A1_A2_A3_A4_A5_A6_A7 = _mm256_permutevar8x32_ps(
			A0_A1_A4_A5_A2_A3_A6_A7, _mm256_setr_epi32(0, 1, 4, 5, 2, 3, 6, 7)
		);
		__m256 B0_B1_B2_B3_B4_B5_B6_B7 = _mm256_permutevar8x32_ps(
			B4_B5_B0_B1_B6_B7_B2_B3, _mm256_setr_epi32(2, 3, 6, 7, 0, 1, 4, 5)
		);

		__m256 D4_D5_C4_C5_D6_D7_C6_C7 = _mm256_shuffle_ps(
			C4_C5_D4_D5_C6_C7_D6_D7, C4_C5_D4_D5_C6_C7_D6_D7, _MM_SHUFFLER(2, 3, 0, 1)
		);
		__m256 C0_C1_C4_C5_C2_C3_C6_C7 = _mm256_blend_ps(
			C0_C1_D0_D1_C2_C3_D2_D3, D4_D5_C4_C5_D6_D7_C6_C7, 0b11001100
		);
		__m256 D4_D5_D0_D1_D6_D7_D2_D3 = _mm256_blend_ps(
			C0_C1_D0_D1_C2_C3_D2_D3, D4_D5_C4_C5_D6_D7_C6_C7, 0b00110011
		);
		__m256 C0_C1_C2_C3_C4_C5_C6_C7 = _mm256_permutevar8x32_ps(
			C0_C1_C4_C5_C2_C3_C6_C7, _mm256_setr_epi32(0, 1, 4, 5, 2, 3, 6, 7)
		);
		__m256 D0_D1_D2_D3_D4_D5_D6_D7 = _mm256_permutevar8x32_ps(
			D4_D5_D0_D1_D6_D7_D2_D3, _mm256_setr_epi32(2, 3, 6, 7, 0, 1, 4, 5)
		);
#else
